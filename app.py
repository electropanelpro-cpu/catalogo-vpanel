...
# --- INTEGRACIÓN DE VENTANA DE ESCRITORIO (Se mantiene pero se desactiva en la nube) ---
try:
    import webview
    GUI_AVAILABLE = True
except ImportError:
...
# ----------------------
# DETECTAR RUTA (Optimizado para la Nube y Local)
# ----------------------
...
# ----------------------
# CONFIGURACIÓN DE IA LOCAL (CONOCIMIENTO_PANEL) - TODO TU CONTENIDO SIGUE AQUÍ
# ----------------------
CONOCIMIENTO_PANEL = {
    "normas": [
...
# --- INICIO (Ajustado para Render) ---
if __name__ == '__main__':
    with app.app_context():
        # Se crean las tablas y los proyectos si no existen
        db.create_all()
        crear_proyectos_iniciales()
    
    # Puerto dinámico para que Render asigne el correcto
    port = int(os.environ.get("PORT", 5000))
    app.run(host='0.0.0.0', port=port)from flask import Flask, render_template, request, redirect, url_for, jsonify, flash, send_from_directory
from flask_sqlalchemy import SQLAlchemy
from flask_mail import Mail, Message
import os
import sys
